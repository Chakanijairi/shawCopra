...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err?.name === "AbortError" ? "Request timed out. Is the backend running on port 8000?" : (err?.message || "Registration failed")
      setError(msg)
    } finally {
      setLoading(false)
    }
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center py-12 px-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        ...
        <div className="flex justify-center gap-8 mb-6">
          <Link to="/login" className="text-gray-500 font-medium pb-1 hover:text-gray-700">
            Login
          </Link>
          <Link to="/register" className="text-gray-900 font-medium pb-1 border-b-2 border-red-500">
            Register
          </Link>
        </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-6">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="role" value="user" checked={role === "user"} onChange={() => setRole("user")} className="w-4 h-4 text-[#664C36] border-gray-300 focus:ring-[#664C36]" />
                <span className="text-gray-700">User</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="role" value="admin" checked={role === "admin"} onChange={() => setRole("admin")} className="w-4 h-4 text-[#664C36] border-gray-300 focus:ring-[#664C36]" />
        ...
        <div className="relative my-6">
          ...
          <p className="relative text-center text-sm text-gray-500 bg-white px-2">Or register with</p>
        </div>
        ...
        <div className="text-center mt-6">
          <Link to="/" className="inline-flex items-center gap-1 px-5 py-2.5 rounded-xl bg-[#664C36] text-white text-sm font-medium hover:bg-[#5a4230] transition-colors">
